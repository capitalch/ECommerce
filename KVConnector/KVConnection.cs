                    ...
                    if (objDictionary.ContainsKey("auth"))
                    {
                        ...
                        if (auth.IndexOf(':') > 0)
                        {
                            ...
                            if (splitAuth.Length == 2)
                            {
                                ...
                                {
                                    email = item;
                                    //get first code from DB against the email
                                    paramsList.Add(new SqlParameter("email", item));
                                    var code = seedDataAccess.ExecuteScalar(SqlResource.GetCodeFromEmail, paramsList);
                                    if (code == null)
                                    {
                                        throw new Exception(Resources.ErrNoCodeForEmail);
                                    }
                                    else
                                    {
                                        item = code.ToString();
                                ...
                                {
                                    email = Util.GetEmailFromCode(seedDataAccess, item);
                                }
                                paramsList.Clear();
                                paramsList.Add(new SqlParameter("item", item));
